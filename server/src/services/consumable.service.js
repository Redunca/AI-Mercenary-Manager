...
async function getConsumable(client, consumableId) {
  const result = await client.query('SELECT * FROM consumables WHERE id = $1', [consumableId])
...
async function hasStashRoom(client, playerId) {
  const capacityResult = await client.query(
    'SELECT inventory_capacity FROM players WHERE id = $1',
    [playerId],
  )
...
async function addToStash(
  client,
  playerId,
  { name, description, rarity, price, effect, effectData, quantity = 1 },
) {
...
async function moveStack(client, playerId, consumableId, destinationShipId, quantity = 1) {
  ...
  if (source.quantity === quantity) {
    ...
    await client.query('UPDATE consumables SET quantity = quantity - $1 WHERE id = $2', [
      quantity,
      source.id,
    ])
  }
  ...
  const inserted = await client.query(
    ...
    [
      ...
      source.price,
      source.effect,
      JSON.stringify(source.effect_data ?? {}),
      quantity,
      destinationShipId,
    ],
  )
...
async function assignToShip(client, playerId, consumableId, shipId, quantity = 1) {
  ...
  if (result) {
    await OperaService.recordOperaAction(client, playerId, 'assign_item_to_ship', {
      shipId,
...
async function consumeFromShipInventory(client, shipId, effect, matchEffectData) {
  ...
  const match = matchEffectData
    ? result.rows.find((row) => matchEffectData(row.effect_data))
    : result.rows[0]
