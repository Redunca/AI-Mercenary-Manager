...
function createFakeClient() {
  ...
  const query = jest.fn(async (sql, params = []) => {
    ...
    if (s.includes('UPDATE players SET next_candidate_id = $1 WHERE id = $2')) {
      Object.assign(
        state.players.find((p) => p.id === params[1]),
        { next_candidate_id: params[0] },
      )
      return { rows: [] }
    }
    ...
    if (
      ...
      return {
        rows: state.players
          ...
          .map((p) => ({
            wallet: p.wallet,
            shop_refresh_at: p.shop_refresh_at,
            shop_refresh_interval_ms: p.shop_refresh_interval_ms,
          })),
      }
    }
    ...
    if (s === 'SELECT shop_rotation_size, shop_refresh_interval_ms FROM players WHERE id = $1') {
      return {
        rows: state.players
          .filter((p) => p.id === params[0])
          .map((p) => ({
            shop_rotation_size: p.shop_rotation_size,
            shop_refresh_interval_ms: p.shop_refresh_interval_ms,
          })),
      }
    }
    ...
    if (s === 'SELECT inventory_capacity FROM players WHERE id = $1') {
      return {
        rows: state.players
          .filter((p) => p.id === params[0])
          .map((p) => ({ inventory_capacity: p.inventory_capacity })),
      }
    }
    if (
      s ===
      ...
      return {
        rows: [
          {
            count: state.consumables.filter(
              (c) => c.player_id === params[0] && c.assigned_to_ship == null,
            ).length,
          },
        ],
      }
    ...
    if (
      s === 'SELECT COUNT(*)::int AS count FROM ships WHERE player_id = $1 AND deleted_at IS NULL'
    ) {
      return {
        rows: [
          {
            count: state.ships.filter((sh) => sh.player_id === params[0] && !sh.deleted_at).length,
          },
        ],
      }
    }
    if (
      ...
      const capacity = state.dockingStations
        .filter((d) => d.player_id === params[0])
    ...
    if (s === 'SELECT next_ship_id FROM players WHERE id = $1') {
      return {
        rows: state.players
          .filter((p) => p.id === params[0])
          .map((p) => ({ next_ship_id: p.next_ship_id })),
      }
    }
    ...
    // ships
    if (
      s === 'SELECT * FROM ships WHERE player_id = $1 AND deleted_at IS NULL ORDER BY created_at'
    ) {
    ...
    if (s === 'SELECT * FROM ships WHERE player_id = $1 AND deleted_at IS NULL ORDER BY id') {
      return {
        rows: state.ships
          .filter((sh) => sh.player_id === params[0] && !sh.deleted_at)
          .sort((a, b) => a.id - b.id),
      }
    }
    if (
      s === 'SELECT * FROM ships WHERE player_id = $1 AND id = $2 AND deleted_at IS NULL' ||
      s === 'SELECT * FROM ships WHERE player_id = $1 AND id = $2 AND deleted_at IS NULL FOR UPDATE'
    ) {
      return {
        rows: state.ships.filter(
          (sh) => sh.player_id === params[0] && sh.id === params[1] && !sh.deleted_at,
        ),
      }
    }
    ...
    if (
      s ===
      'UPDATE ships SET crew = $1 WHERE player_id = $2 AND id = $3 AND deleted_at IS NULL RETURNING *'
    ) {
      const [crew, playerId, id] = params
      const sh = state.ships.find(
        (sh) => sh.player_id === playerId && sh.id === id && !sh.deleted_at,
      )
      if (sh) sh.crew = crew
      return { rows: sh ? [sh] : [] }
    }
    if (
      s ===
      'UPDATE ships SET status = $1 WHERE player_id = $2 AND id = $3 AND deleted_at IS NULL RETURNING *'
    ) {
      const [status, playerId, id] = params
      const sh = state.ships.find(
        (sh) => sh.player_id === playerId && sh.id === id && !sh.deleted_at,
    ...
    if (
      s ===
      "UPDATE ships SET status = 'destroyed', deleted_at = NOW() WHERE player_id = $1 AND id = $2 RETURNING *"
    ) {
    ...
    if (
      s === 'UPDATE ships SET stats = $1, status = $2 WHERE player_id = $3 AND id = $4 RETURNING *'
    ) {
    ...
    if (
      s ===
      'UPDATE ships SET name = $3 WHERE player_id = $1 AND id = $2 AND deleted_at IS NULL RETURNING *'
    ) {
    ...
    if (s.includes('INSERT INTO docking_stations')) {
      const station = {
        id: state.dockingStations.length + 1,
        player_id: params[0],
        capacity: params[1],
      }
    ...
    if (s.startsWith('SELECT si.*, sr.remaining_stock FROM shop_items si')) {
      ...
      const joined = state.shopRotation
        ...
        .map((r) => ({
          ...state.shopItems.find((i) => i.id === r.shop_item_id),
    ...
    if (
      ...
      state.shopRotation.find(
        (r) => r.player_id === player_id && r.shop_item_id === shop_item_id,
      ).remaining_stock -= 1
      return { rows: [] }
    }
    if (
    ...
    if (
      ...
      return {
        rows: state.consumables.filter(
          (c) => c.player_id === params[0] && c.name === params[1] && c.assigned_to_ship == null,
        ),
      }
    }
    if (
      s === 'SELECT * FROM consumables WHERE player_id = $1 AND name = $2 AND assigned_to_ship = $3'
    ) {
      return {
        rows: state.consumables.filter(
          (c) =>
            c.player_id === params[0] && c.name === params[1] && c.assigned_to_ship === params[2],
        ),
      }
    }
    ...
    if (s === 'SELECT * FROM consumables WHERE assigned_to_ship = $1 AND effect = $2 ORDER BY id') {
      return {
        rows: state.consumables.filter(
          (c) => c.assigned_to_ship === params[0] && c.effect === params[1],
        ),
      }
    }
    ...
    if (
      s ===
      'SELECT * FROM consumables WHERE player_id = $1 AND assigned_to_ship IS NULL ORDER BY created_at'
    ) {
      return {
        rows: state.consumables.filter(
          (c) => c.player_id === params[0] && c.assigned_to_ship == null,
    ...
    if (
      ...
      const c = {
        ...
        price,
        effect,
        effect_data: JSON.parse(effect_data),
        quantity,
        assigned_to_ship,
        created_at: new Date(),
      }
    ...
    if (s.includes('SELECT message FROM log_entries')) {
      ...
      const matches = state.logEntries.filter(
        (l) => l.player_id === playerId && sameId(l.mission_id, missionId),
      )
      return {
        rows: matches
          .slice(-limit)
...
describe('Full flow: buy ship -> hire -> assign crew -> buy item -> assign inventory -> start mission', () => {
  ...
  test('the whole chain succeeds and leaves the game in the expected state', async () => {
    ...
    expect(state.ships.find((s) => s.id === newShipId)).toMatchObject({
      status: 'docked',
      crew: [],
    })
    ...
    const assignInvResult = await ConsumableService.assignToShip(
      invClient,
      1,
      consumableId,
      newShipId,
  ...
  test('refuses to start the mission if the loaded item is not actually a speed-boost consumable', async () => {
    ...
    const invClient = await pool.connect()
    const inv = await ConsumableService.assignToShip(
      invClient,
      1,
      buyResult.consumable.id,
      shipId,
      1,
    )
    invClient.release()

    const mission = (await GameService.getGameState()).missions.find(
