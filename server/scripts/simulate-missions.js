...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    if (/^\d+$/.test(arg)) { runs = Number(arg); continue; }
    const seedMatch = arg.match(/^--seed=(\d+)$/);
    if (seedMatch) { seed = Number(seedMatch[1]); continue; }
    const diffMatch = arg.match(/^--difficulty=(\w+)$/);
    if (diffMatch) { difficulty = diffMatch[1].toUpperCase(); continue; }
    const crewMatch = arg.match(/^--crew=(\d+)$/);
    if (crewMatch) { crewSize = Number(crewMatch[1]); continue; }
    const archMatch = arg.match(/^--archetype=([\w-]+)$/);
    if (archMatch) { archetype = archMatch[1]; continue; }
  }
...
function simulateMission(mission, crew) {
  ...
  for (const event of mission.events) {
    ...
    if (activeCrew.length === 0) { stats.failed = true; stats.partyWiped = true; break; }
  ...
  for (const diff of difficulties) {
    ...
    for (let i = 0; i < runs; i++) {
      ...
      for (const outcome of result.eventOutcomes) {
        if (!agg.eventOutcomes[outcome.type]) agg.eventOutcomes[outcome.type] = { success: 0, failure: 0 };
...
function printReport(results, { runs, crewSize, archetype }) {
  ...
  for (const [type, o] of Object.entries(combined)) {
    console.log(`  ${type.padEnd(16)} ${pct(o.success, o.success + o.failure)} (${o.success + o.failure} occurrences)`);
  }
}
